...
class DataPointService:
    @staticmethod
    def get_data_points(db: Session, skip: int = 0, limit: int = 100) -> List[DataPoint]:
    ...
    def get_data_point(db: Session, data_point_id: int) -> Optional[DataPoint]:
        ...
        return db.query(DataPoint).filter(DataPoint.id == data_point_id).first()
    ...
    def delete_data_point(db: Session, data_point_id: int) -> bool:
        """Delete a data point"""
        db_data_point = db.query(DataPoint).filter(
            DataPoint.id == data_point_id).first()
