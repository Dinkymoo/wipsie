...
def poll_data(self, source_url: str, data_type: str):
    ...
    try:
        # Update task state
        self.update_state(
            state="PROGRESS", meta={"status": "Starting data polling"}
        )
        ...
        self.update_state(
            state="FAILURE", meta={"status": "Failed", "error": str(e)}
        )
        raise
...
def process_task(self, task_id: int, task_data: dict):
    ...
    try:
        self.update_state(
            state="PROGRESS", meta={"status": f"Processing task {task_id}"}
        )
...
def send_notification(
    ...
    try:
        self.update_state(
            state="PROGRESS",
            meta={"status": f"Sending {notification_type} to {recipient}"},
        )
